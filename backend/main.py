...
def research_validation_page() -> str:
    ...
    return f"""<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>研究与执行验证</title><style>{_product_css()}</style></head><body><header><h1>研究与执行验证</h1><p class="subtle">供用户按 Research → Execution → Shadow 顺序检查 V12 候选。所有内容均不用于直接交易。</p>{_primary_navigation()}</header><main><section class="alert"><span class="tag blocked">全历史仍有上市前缺口</span><h2>Execution Validation 当前状态</h2><p>14个研究资产均已有映射，覆盖率和收益差门槛已通过；剩余原因来自ETF尚未上市的历史月份，不代表当前仍缺少代理。</p><ul>{reasons}</ul></section><section><h2>覆盖率和缺口怎么理解</h2><div class="grid-3"><div class="card"><h3>非现金研究权重覆盖率</h3><p><strong>{float(execution.get('tradable_weight_coverage',0) or 0):.2%}</strong></p><p>分子是可交易 ETF 权重，分母是研究组合的非现金权重，不是整个组合资产。</p></div><div class="card"><h3>全组合可交易权重</h3><p><strong>{float(execution.get('tradable_weight_coverage_total_portfolio',0) or 0):.2%}</strong></p><p>以研究组合全部权重为分母，表示组合中实际落到 ETF 的权重。</p></div><div class="card"><h3>全历史缺口月份</h3><p><strong>{float(execution.get('binary_any_gap_month_ratio',0) or 0):.2%}</strong></p><p>某月只要出现任何上市前缺口就计入；不表示当月整个组合都不能交易。</p></div></div><p class="subtle">平均缺口权重 {float(gaps.get('average_gap_weight',0) or 0):.2%}，最大缺口权重 {float(gaps.get('max_gap_weight',0) or 0):.2%}。门槛未被放宽。</p></section><section><h2>四个当前入口</h2><div class="grid-2">{card_html}</div></section><section><h2>正确阅读顺序</h2><ul><li>Research 判断全收益指数配置逻辑是否成立。</li><li>Execution V1 检查真实ETF前复权收益、上市日期和映射偏差。</li><li>Shadow 只展示最近一期月末目标快照，不能替代V11或直接下单。</li><li>weak/low映射表示人工批准的近似代理，不是直接跟踪。</li></ul></section></main><footer>研究与执行验证 · 非交易指令</footer>{_unified_shell_scripts()}</body></html>"""
...
def execution_backtest_v2_page() -> str:
    ...
    return f"""<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>Execution Engine V2 B1</title><style>{_product_css()}</style></head><body><header><h1>Execution Engine V2 B1</h1><p class="subtle">动态可投资时间线实验，用来观察 ETF 晚上市和缺价时组合如何处理。</p>{_primary_navigation()}</header><main><section class="alert"><span class="tag warn">实验验证</span><h2>V1 仍是正式执行验证口径</h2><p>本页不进入 Current Decision，不替代 V1，不生成订单、份额、金额或目标价。</p></section><section><h2>回测时间范围</h2><p>研究期、V2 主交易日历期、首次实际持有 ETF 日期和与 V1 的严格共同日期期分别列示，避免把不同区间直接混为一谈。</p><div class="table-wrap"><table><tbody>{period_rows}</tbody></table></div></section><section><h2>V2 实验指标</h2><p>本阶段交易成本、滑点和现金收益均为 0；指标只能用于验证时间线和记账逻辑。</p><div class="table-wrap"><table><tbody>{metric_rows}</tbody></table></div></section><section><h2>可投资时间线摘要</h2><p>每只 ETF 独立判断；不会为了凑齐所有 ETF 而统一删除日期。持仓缺价时保留最近验证估值，未完成调仓单列为延期差额。</p><div class="table-wrap"><table><tbody>{timeline_rows}</tbody></table></div></section><section><h2>首个执行事件</h2><p>研究信号在下一个交易日执行。页面区分请求目标、实际调仓后权重和延期调整；冻结持仓不会重复计入现金缺口。</p><div class="table-wrap"><table><tbody>{event_rows}</tbody></table></div></section><section><h2>V1 与 V2 中性对照</h2><p>严格共同日期视图使用完全相同的日期集合；主日历对齐视图对 V1 只做分析性前向持有。差异不能用于认定 V2 更优或替代 V1。</p><div class="table-wrap"><table><tbody>{comparison_rows}</tbody></table></div></section><section><h2>当前限制</h2><table><tbody>{warning_rows}</tbody></table><p><a class="button" href="/api/research/execution-backtest-v2">查看已校验的只读 JSON</a></p></section></main><footer>实验性执行验证 · 非交易指令</footer>{_unified_shell_scripts()}</body></html>"""
...
def research_universe_page() -> str:
    ...
    <body>
      <header>
        ...
        <p>核对研究指数、ETF 中文名称、执行代理和数据可用性；用于研究审计，不是交易指令。</p>{_primary_navigation()}
      </header>
      <main>
        <section>
...
def research_backtest_page() -> str:
    ...
    <body>
      <header>
        <h1>V12 研究资产历史验证</h1>
        <p>用途：用全收益指数检验月度配置逻辑和加入 480092.CNI 后的变化。它不等于 ETF 收益，不替代 V11；当前结果只可作为 Execution-Aware Shadow 的候选研究依据。</p>{_primary_navigation()}
      </header>
      <main>
        <section>
          <h2>Status</h2>
...
def execution_backtest_page() -> str:
    ...
    non_executable = "\n".join(_message_rows([str(row.get("research_asset_id")) for row in report.get("non_executable_assets", [])], empty="当前没有不可执行资产"))
    no_proxy = "\n".join(_message_rows([str(row.get("research_asset_id")) for row in report.get("unmapped_assets", [])], empty="当前没有无获批代理资产"))
    low_quality = "\n".join(_message_rows([str(row.get("research_asset_id")) for row in report.get("low_quality_proxy_assets", [])], empty="当前没有因低质量被排除的代理"))
    ...
    counter_rows = "\n".join(_mapping_rows({"status":counterfactual.get("status"),"evidence_use":counterfactual.get("evidence_use"),"release_scope":counterfactual.get("release_scope"),"baseline_contract_verification":counterfactual.get("baseline_contract_verification"),"input_contract_verification":counterfactual.get("input_contract_verification"),"common_period":counterfactual.get("common_comparison_period"),"impact":counterfactual.get("impact"),"delta_contract":counterfactual.get("delta_contract"),"decision":counterfactual.get("decision")}, empty="No counterfactual report recorded"))
    ...
    return f"""<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>ETF 执行验证</title><style>{_report_page_css()}</style></head><body><header><h1>ETF 执行验证</h1><p>用途：把 V12 研究指数月末目标映射为真实 ETF 前复权收益，检查覆盖率、上市前缺口和收益偏差。低质量近似代理会保留原始风险标签；本页不是交易指令。</p>{_primary_navigation()}</header><main>
