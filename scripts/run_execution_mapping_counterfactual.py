...
p=argparse.ArgumentParser();p.add_argument('--provider',choices=['local'],default='local');a=p.parse_args();proposal=json.loads(PROPOSAL.read_text(encoding='utf-8'));research=load_research_backtest_report();assets=load_execution_universe();baseline,counter,common,impact=run_mapping_counterfactual(research,load_asset_mappings(),proposal['proposals'],load_execution_price_dataset(assets),assets,data_provider='tushare');collisions=proposal_collisions(proposal['proposals'],research['monthly_allocations']);reasons=[]
...
report={"available":True,"data_provider":"tushare","baseline":baseline,"counterfactual":counter,"common_comparison_period":common,"impact":impact,"proxy_collision_diagnostics":collisions,"decision":{"ready_for_manual_mapping_approval":not reasons,"reasons":reasons,"warning":"Counterfactual proposal only; asset_mapping.json has not been modified."}};write_report(report,COUNTER);print({"ready":not reasons,"impact":impact})
