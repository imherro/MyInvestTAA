...
The verified counterfactual is copied into the committed release as an advanced audit artifact with `release_scope=committed_release`; the mutable root report remains `mutable_pre_release`. The formal API reads only through full committed-release verification, including the manifest, acceptance report, commit marker, artifact hashes, release hashes, and required gates. Integrity failure returns `available=false` and never falls back to the mutable report. `counterfactual_integrity` is a required System Acceptance gate. Counterfactual evidence remains non-production and cannot approve a mapping or affect Current Decision. Return, drawdown, coverage, any-gap, and cash-drag changes use `fraction_point` internally and `percentage_points` for display; Sharpe remains unitless.
