def build_execution_mapping(research_allocations, asset_mappings, execution_universe, *, allow_low_quality_proxy=False):
    ...
    for asset_id in sorted(used):
        mapping=by_research.get(asset_id); quality=mapping.mapping_quality if mapping else "none"; proxy=mapping.primary_execution_proxy if mapping else None
        allowed=bool(proxy and proxy in execution_ids and quality in {"high","medium"} or (allow_low_quality_proxy and proxy and proxy in execution_ids and quality=="low"))
        reason="mapped" if allowed else ("low_quality_proxy_excluded" if quality=="low" else "mapping_unavailable")
        rows.append({"research_asset_id":asset_id,"proxy_id":proxy if allowed else None,"mapping_quality":quality,"executable":allowed,"reason":reason})
    return rows
