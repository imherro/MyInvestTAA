...
def test_unknown_asset_approval_api_is_available_false():
    response = CLIENT.get("/api/research/execution-mapping-approval-package/UNKNOWN")
    assert response.status_code == 200
    assert response.json()["available"] is False


def test_approval_package_api_existing():
    response = CLIENT.get(f"/api/research/execution-mapping-approval-package/{TARGET_ASSET_ID}")
    assert response.status_code == 200
    assert response.json()["ready_for_explicit_human_decision"] is True
...
def test_approval_api_missing_report_does_not_500(monkeypatch):
    monkeypatch.setattr(
        "backend.main.load_mapping_approval_package",
        lambda asset_id=None: {"available": False, "message": "missing"},
    )
    response = CLIENT.get(f"/api/research/execution-mapping-approval-package/{TARGET_ASSET_ID}")
    assert response.status_code == 200
    assert response.json()["available"] is False
...
@pytest.mark.parametrize(
    "section",
    [
        "Selective Mapping Approval Package",
        "Exact Drawdown Reconciliation",
        "ETF Tracking Index Evidence",
        "Exposure Differences",
        "Existing Proxy Collision",
        "Ready for Explicit Human Decision?",
        "Frozen Research-Only and Rejected Proposals",
    ],
)
def test_execution_page_has_task_031_sections(section):
